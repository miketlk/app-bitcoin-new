...
Given a PSET and a registered wallet (or a standard one), sign all the inputs that are owned by that wallet. Command name, SIGN_PSBT, and its opcode were unchanged for compatibility with the baseline Bitcoin application.
...
The fingerprint is necessary to fill the key origin information for some PSET fields, or to create wallet descriptors.
...
| Length | Description     |
|--------|-----------------|
| `32`   | The master private blinding key |

#### Description

The master private blinding key is derived from the seed using the method described in SLIP-0077.
...
| Length          | Name            | Description |
|-----------------|-----------------|-------------|
| `<variable>`    | `script_length` | The length of the script field (unsigned varint) |
| `script_length` | `script`        | The scriptPubKey used to derive blinding key |

The scriptPubKey (`script` field) is provided in raw binary form. At this time, scriptPubKey cannot be longer than 252 bytes; therefore, the `script_length` field is always encoded as 1 byte.

**Output data**

| Length | Description     |
|--------|-----------------|
| `32`   | The private blinding key |

#### Description

The secure device first derives the master private blinding key from stored seed using the method described in SLIP-0077. Then this master blinding key is used to derive the blinding key depending on provided scriptPubKey.
...
All the current commands use a commit-and-reveal approach: the APDU that starts the protocol (first message) commits to all the relevant data (for example, the entirety of the PSET), by using hashes and/or Merkle trees. Any time the client is asked to reveal some committed information, the app does not consider it trusted:
