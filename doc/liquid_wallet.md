# Wallet policy with Liquid-specific extensions
...
`BLINDING_KEY` expressions specify master blinding key derivation according to ELIP 150 and ELIP 151:
...
- `<KEY>` which is a key expression as described in BIP 380, with the exception that uncompressed and x-only keys are not allowed, moreover single private keys are encoded hex instead of WIF. The blinding key is derived by a Taproot-style tweak as specified in ELIP 150.
- `elip151` indicates that descriptor blinding key to be used is a view key deterministically derived from the ordinary descriptor via ELIP 151.
